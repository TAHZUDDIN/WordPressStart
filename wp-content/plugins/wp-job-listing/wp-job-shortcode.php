<?php
  ...
  function dwwp_list_job_by_location( $atts, $content = null){
        ...
        if(! isset( $atts['location'])){
          return '<p class="job-error">You must provide a location for this shortcdde to work . </p>';
        }
       ...
       if($jobs_by_location-> have_posts()):
           ...
           $display_by_location .= '</div>';
               ...
               $display_by_location = sprintf(__( '<p class="job-error">Sorry, no jobs listed in %s where found.</p>'), esc_html__( ucwords( str_replace('_', ' ', $atts[ 'location']))));
           ...
           if( $jobs_by_location-> max_num_pages > 1  && is_page() ){
               ...
               $display_by_location .= get_next_posts_links( __('<span class="meta-nav">$larr;</span> Previous' ), $jobs_by_location-> max_num_pages);
               $display_by_location .= '</div>';
